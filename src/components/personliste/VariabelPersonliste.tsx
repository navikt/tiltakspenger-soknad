                ...
                <fieldset style={{ marginTop: "1rem" }}>
                    ...
                    <div key={item.id}>
                        <Controller
                            name={`${name}.${index}`}
                            control={control}
                            defaultValue={{fornavn: "", etternavn: "", fdato: "", bodstedsland: ""}}
                            render={({ field }) => (
                                <>
                                    <TextField
                                        ...
                                        onChange={event =>
                                            field.onChange({ ...field.value, fornavn: event.target.value })
                                        }
                                    />
                                    <TextField
                                        ...
                                        onChange={event =>
                                            field.onChange({ ...field.value, etternavn: event.target.value })
                                        }
                                    />
                                    <Datovelger onDateChange={(dato) => field.onChange({fdato: dato?.toISOString().split("T")[0] ?? ""})} label={"Test"}/>
                                    <TextField
                                        label={"Bostedsland"}
                                        value={field.value.bostedsland || ""}
                                        onChange={event =>
                                            field.onChange({ ...field.value, bostedsland: event.target.value })
                                        }
                                    />
                                </>
                            )}
